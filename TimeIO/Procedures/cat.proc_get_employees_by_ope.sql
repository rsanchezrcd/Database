...
GO
CREATE procedure [cat].[proc_get_employees_by_ope]
	@ope char(36)
	,@dep char(36) = null
	,@per char(36) = null 
as begin
	...
	if @dep is null begin
		...
		where do.id_operator = @ope and cat.func_isactive_bydt(@ini, e.employee_id) = 1--e._status = 1
		...
		inner join cat.posicion p with (nolock) on (e.posicion_id = p.posicion_id)
		inner join cat.locacion l with (nolock) on (e.locacion_id = l.locacion_id)
		where do.id_operator = @ope and cat.func_isactive_bydt(@ini, e.employee_id) = 1 and d.id_departamento = @dep
		order by [_alter_id] asc;
	end;
end;
GO
